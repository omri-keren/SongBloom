...
from SongBloom.models.musicgen.conditioners.base import JointEmbedCondition
...
def main():
    ...
    for test_sample in input_lines:
        ...
        # Construct JointEmbedCondition (assuming model expects 'joint_condition' key)
        joint_condition = JointEmbedCondition(
            wav=prompt_wav.to(dtype),
            text=[lyrics],
            length=torch.tensor([prompt_wav.shape[-1]]).long(),
